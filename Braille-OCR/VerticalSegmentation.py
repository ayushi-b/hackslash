...
def vertical_segmentation(base_path, n_lines):
    ...
    for i in range(n_lines):
        ...
        cv2.imwrite(base_path + 'cannyEdge1.jpg', edges)
        ...
        cv2.imwrite(base_path + 'hough1.jpg', img)
        ...
        hough_lines = cv2.imread(base_path + 'hough1.jpg')
        p = []
        ...
        for i in range(len(pxl)):
            # print(type(pxl[i].shape[1]))
            # print(pxl[i].shape[1])
            # print(sum(o), min(o), len(o))
            # print("\n")
        ...
        for i in range(len(pxl)):
            ...
            cv2.imwrite(base_path + 'character[' + str(i) + '].jpg', pxl[i])

            # dividing each image into 6 equal rois
            w.append((pxl[i][0:(pxl[i].shape[0]) // 3, 0:(pxl[i].shape[1]) // 2],
                      pxl[i][(pxl[i].shape[0]) // 3:2 * ((pxl[i].shape[0]) // 3), 0:(pxl[i].shape[1]) // 2],
                      pxl[i][2 * ((pxl[i].shape[0]) // 3):pxl[i].shape[0], 0:(pxl[i].shape[1]) // 2],
                      pxl[i][0:(pxl[i].shape[0]) // 3, (pxl[i].shape[1]) // 2:(pxl[i].shape[1])],
                      pxl[i][(pxl[i].shape[0]) // 3:2 * ((pxl[i].shape[0]) // 3), (pxl[i].shape[1]) // 2:(pxl[i].shape[1])],
                      pxl[i][2 * (pxl[i].shape[0]) // 3:pxl[i].shape[0], (pxl[i].shape[1]) // 2:(pxl[i].shape[1])]))
        ...
        alpha = {
            'a': '100000', 'c': '100100', 'b': '110000', 'e': '100010', 'd': '100110', 'g': '110110', 'f': '110100',
            'i': '010100', 'h': '110010', 'k': '101000', 'j': '010110', 'm': '101100', 'l': '111000', 'o': '101010',
            'n': '101110', 'q': '111110', 'p': '111100', 's': '011100', 'r': '111010', 'u': '101001', 't': '011110',
            'w': '010111', 'v': '111001', 'y': '101111', 'x': '101101', 'z': '101011'
        }
        n = {
            '0': '010110', '1': '100000', '2': '110000', '3': '100100', '4': '100110', '5': '100010', '6': '110100',
            '7': '110110', '8': '110010', '9': '010100'
        }
        c = {',': '010000', '\'': '000010', '.': '010011', '!': '011010', '?': '011001', ';': '011000', ' ': '000000'}

        for ch, valu in alpha.items():
            if cvt[0] == valu:
                f.write(ch)
                continue

        for ch, valu in c.items():
            if cvt[0] == valu:
                f.write(ch)
                continue
